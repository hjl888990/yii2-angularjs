...
//----这里"@" ↓↓↓ 把报错屏蔽了，需要删除"@"才能看见上面的报错,
        @$this->_socket = stream_socket_client(
                $this->unixSocket ? 'unix://' . $this->unixSocket : 'tcp://' . $this->hostname . ':' . $this->port, $errorNumber, //----这两个变量没有提前定义一直报undefined
        ...
        if ($this->_socket) {
            ...
            \Yii::error("Failed to open redis DB connection ($connection): $errorNumber - ".iconv("GB2312","UTF-8",$errorDescription), __CLASS__);
            $message = YII_DEBUG ? "Failed to open redis DB connection ($connection): $errorNumber - ".iconv("GB2312","UTF-8",$errorDescription) : 'Failed to open DB connection.';
